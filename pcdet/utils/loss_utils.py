...
def mdn_loss(pi, sigma, mu, target):
    ...
    Returns:
        B O
    """
    sigma = torch.clamp_min(sigma, min=1e-7) # NOTE without this cause nan
    pi = torch.clamp_min(pi, min=1e-7).unsqueeze(-1).expand_as(sigma)
    target = target.unsqueeze(1).expand_as(sigma)
    
    model_dist = dist.Normal(mu, sigma)
    log_likelihood = model_dist.log_prob(target)
    weighted_log_prob = torch.log(pi) + log_likelihood
